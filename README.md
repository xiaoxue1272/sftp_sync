...
| 配置项 | 类型 | 默认值 | 说明 | 填写方式 |
|--------|------|--------|------|----------|
| `temp_dir` | 字符串 | 无 | 临时文件存储目录，下载时的临时文件会先保存到这里 | 填写绝对路径或相对路径，如 `"./temp"` 或 `"/path/to/temp"` |
| `log_dir` | 字符串 | 无 | 日志文件存储目录 | 填写绝对路径或相对路径，如 `"./logs"` |
| `batch_size` | 整数 | 100 | 每批处理的文件数量 | 填写数字，如 `1000` |
| `max_retries` | 整数 | 3 | 单个文件下载失败最大重试次数 | 填写数字，如 `5` |
| `max_threads` | 整数 | 5 | 并发下载线程数 | 填写数字，如 `10` |
| `retry_delay` | 整数 | 30 | 重试间隔时间（秒） | 填写数字，如 `60` |
| `preserve_timestamps` | 布尔 | true | 是否保持文件时间戳 | 填写 `true` 或 `false` |
| `connect_timeout` | 整数 | 30 | 连接超时时间（秒） | 填写数字，如 `60` |
| `transfer_timeout` | 整数 | 300 | 传输超时时间（秒） | 填写数字，如 `600` |
| `batch_delay` | 整数 | 无 | 批次间延迟时间（秒） | 填写数字，如 `30`，可选配置项 |
...
| `pattern` | 字符串 | 否 | 正则表达式匹配子路径 | 使用正则表达式匹配子路径，如 `".*\\.txt$"`、`"data_.*\\.csv$"`、`".*\\..*$"`（全部文件）；注意：此字段用于匹配完整路径而非仅文件名 |
| `recursive` | 布尔 | 否 | 是否递归下载子目录  | `true` 递归下载，`false` 仅当前目录，默认 `false`                                                    |
| `check_by_md5` | 布尔 | 否 | 是否进行MD5校验  | `true` 进行校验，`false` 不校验，默认 `false`                                                      |
...
- `temp_dir`: 临时文件存储目录，下载的临时文件会先保存到这里，下载完成后移动到备份目录

#### 3. 文件匹配填写说明

- `pattern`: 使用正则表达式匹配完整路径
  - `*.*`: 所有文件
  - `*.txt`: 所有txt文件
  - `data_*.csv`: 以"data_"开头的CSV文件
  - `report_2023*.pdf`: 2023年的PDF报告文件
...
- `check_by_md5: true`: 下载完成后会进行MD5校验，确保文件完整性
- `check_by_md5: false`: 不进行MD5校验，速度更快但不保证完整性
...
- 下载的文件会先保存到 [temp_dir](file:///D:/projects/py_scrpits/sftp_sync/main.py#L31-L31) 指定的目录
- 临时文件名使用 `.tmp` 扩展名
...
- 日志文件保存在 [log_dir](file:///D:/projects/py_scrpits/sftp_sync/main.py#L32-L32) 指定的目录
- 每次运行会创建一个新的日志文件
- 日志包含详细的同步过程信息
...
5. **并发控制**：合理设置 [max_threads](file:///D:/projects/py_scrpits/sftp_sync/main.py#L31-L31) 以避免服务器负载过高
...
### Q: 如何动态设置路径？
A: 使用 `${expression}` 语法，如 `${datetime.now() - timedelta(days=1)}` 来生成昨天的日期。

### Q: 如何只同步特定文件？
A: 使用 [pattern](file:///D:/projects/py_scrpits/sftp_sync/main.py#L31-L31) 配置项指定正则表达式模式，如 `".*\\.txt$"` 只同步文本文件。

### Q: 下载失败怎么办？
A: 程序会自动重试，重试次数由 [max_retries](file:///D:/projects/py_scrpits/sftp_sync/main.py#L31-L31) 配置项控制。
